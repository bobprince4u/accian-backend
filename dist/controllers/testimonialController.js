...
const createTestimonial = async (req, res, next) => {
    try {
        const { clientName, clientPosition, clientCompany, testimonialText, rating, projectId, featured, imageUrl, } = req.body;
...
const updateTestimonial = async (req, res, next) => {
    try {
        ...
        const setClause = keys
            ...
            .join(", ");
        const result = await (0, database_1.query)(`UPDATE testimonials SET ${setClause} WHERE id = $${keys.length + 1} RETURNING *`, [...values, id]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message: "Testimonial not found",
